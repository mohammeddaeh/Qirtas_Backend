...
| `src/features/inventory/` | **المخزون والموردون (3-أ، 2026-09-23)**. **دفتر حركات يُضاف ولا يُعدَّل** هو السجل، و`stock_balances` كاشه يُكتب بنفس المعاملة مع قفل الصف (`FOR UPDATE`). التكلفة **متوسط مرجّح متحرك بالليرة والدولار** لكل (متغيّر × فرع) — الاستلام وحده يحرّكها، والصرف يحمل المتوسط الذي غادر عليه. الاستلام **بفاتورة شراء من مورد** تُرحَّل فوراً (سطر بوحدته كما كُتب + بوحدة الأساس)، مع **طبقات استلام** لتاريخ الصلاحية ولتقرير FIFO لاحقاً بلا migration. التلف/الفقد بسبب إلزامي: تحت حدّ القيمة يُرحَّل فوراً وفوقه ينتظر `inventory.approve` **بنطاق فرع المستند**. ترقيم متصل لكل (فرع × نوع). القواعد الصافية بـ`services/stock-rules.ts` (٢٣ اختباراً). **و(3-ب)**: النقل بين الفروع (البضاعة بالطريق لا تخصّ أي فرع، وآلة الحالات دالة صافية والردّ يحمل `next_states`) والجرد الأعمى (رصيد النظام يُخزَّن لحظة كل مسحة، ولا يُرسَل ما دام المحضر مفتوحاً) وحراسة الحذف عبر `core/records/` (منتج له حركات يُرفض حذفه `409` بدل `500` من مفتاح `RESTRICT`). **و(3-ج)**: مسودات الفروع — المسودة **صف بجدول المنتجات** (علمان لا جدول ثانٍ، فلا ينتقل مخزون بين جدولين يوم يُعتمد) تُستلم عليها البضاعة ولا تُباع، ثم تُعتمد أو **تُدمج والمخزون يتبعها** عبر منفذ `core/records/variant-merge.ts` (الكتالوج لا يستورد المخزون) أو تُحذف ما دامت فارغة. والمرتجع للمورد **بفاتورة** ومقيّم بتكلفتها (لا بمتوسط الرف) ومسقوف بما جاء بها وبما على الرف (`services/return-rules.ts`، ١١ اختباراً). **ولأجل الفرونت (3-د)**: `GET /inventory/settings` يحمل `branches` معه (شاشات المخزون تسأل عن الفرع قبل أي رقم، وقاعدة `Features → Features ❌` هناك تمنع قراءة موديول الفروع)، و`GET /inventory/document-items?branch_id&search` يردّ ما يصلح لسطر مستند بوحداته **وبرصيد الفرع منه**. **وأُصلح 2026-09-24 بالتجريب الحيّ**: `postMovements` كانت تقرّر بـ`qtyBase >= 0 && line.cost`، فكلُّ **إدخالٍ بلا تكلفة** يسقط إلى `applyIssue` **فيُخصم بدل أن يُضاف** — مرتجعُ الزبون يُنقص الرصيد، **وفائضُ الجرد يزيد النقص** (والتعليق فوقه يقول عكس ما يفعله الكود). صارت تقرّر **بالإشارة**، وأُضيفت `applyReturn` (الكمية ترتفع والمتوسط **لا يتحرّك**: القطع كانت ملكنا أصلاً، ووزنُها بصفر يجرّ التقييم لأسفل مع كل مرتجع). العقد: [docs/rest_api.md](docs/rest_api.md) §22–24 |
| `src/features/sales/` | **نقطة البيع والفاتورة (7-أ، 2026-09-24)** — `orders_delivery.md` §الفوترة. **السلّة صفٌّ بالخادم بلا رقم وبلا حجز**: الرقم يُصرَف عند السداد وحده، ورقمٌ يُحجز لسلّةٍ تُلغى هو الفجوة التي يوجد الترقيم المتسلسل لمنعها. **والسطر لقطةٌ كاملة** (الاسم وSKU والوحدة والسعر وخصم العرض واسمه والضريبة والتكلفة منسوخة) — فاتورةٌ تقرأ الكتالوج تروي كل شهر قصةً مختلفة عن اليوم نفسه. **والمجاميع حيّةٌ ما دامت مفتوحة ومجمَّدةٌ بعد السداد.** القواعد كلها صافية (`sale-rules.ts`، ١٧ اختباراً كل حالة مع نقيضها): **الضريبة تُستخرَج من داخل السعر** (`× r/(100+r)`) لا تُضاف فوقه · خصم الكاشير **يُوزَّع على السطور** وفرقُ التقريب يُسوَّى فلا يختلف الإيصال عن نفسه · **الباقي من النقد وحده** · والمتاح آجلاً `limit + balance`. **وسقف الخصم رقمٌ على الدور لا مفتاح صلاحية**، بثلاثة أجوبة (مسموح · يحتاج مديراً · مرفوض مهما وافق أحد)، والموافقة **ببريد المدير وكلمة مروره على نفس الجهاز** بلا إصدار جلسة، ويُفحص **بسقفه وبأن حسابه فعّال**. **والسداد أربعة أشياء بمعاملة واحدة**: الرقم · التجميد · إخراج البضاعة عبر منفذ `core/stock/stock-port.ts` (المخزون يسجّله؛ فصلُ الكتابتين يترك فاتورةً لبضاعة ما زالت بالدفتر) · وقيد دفتر الزبون. **ودفتر الزبون يُضاف إليه ولا يُعدَّل** والرصيد مجموعُ صفوفه، والإشارة هي المعنى (موجبٌ له · سالبٌ عليه). **و(7-ج) المرتجع من الزبون**: مفتاحٌ مستقل `sales.refund` (البيع يُدخل مالاً والمرتجع يُخرجه)، و**الإرجاع بالسعر المدفوع فعلاً** (مجموع السطر ÷ كميته — لا المعلن ولا سعر اليوم)، و**السقف من الخادم** بأسطرٍ مقفلة فلا يُرجع جهازان القطعة نفسها. **والكاشير يقول حال القطعة وهي بيده**: الصالح يعود للرفّ والتالف **لا يدخل الرصيد** والمال يُرَدّ كاملاً. **والمهلة واحدة للشركة**، وبعدها موافقةُ مديرٍ على نفس الجهاز لا رفضٌ قاطع، و`beyond_window` **يُحفظ ولا يُشتقّ**. والبضاعة تدخل مخزون **الفرع المستلِم** (الإرجاع بأي فرع)، والترقيم **مستقل** (`MZ-R-…`) فلا يقطع ترقيم الفواتير. **و(8-أ) السلّة والطلب الإلكتروني**: السلّة **بالخادم** (يجدها على هاتفه وحاسوبه) وواحدة لكل (زبون × فرع)، **ولا تحجز شيئاً** — سلاتٌ متروكة تجمّد المخزون فيقرأ الكاشير رفّاً ممتلئاً لا يبيع منه. **والتأكيد يحجز** عبر منفذ `core/stock/reservation-port.ts` بمعاملة ذرّية وفحصٍ **داخل قفل الصفّ** (بلا القفل يُوعَد اثنان بآخر قطعة)، والرفض يحمل **العدد المتاح** فلا يخفّض الزبون كميته تخميناً. **ولا يُقال للزبون «غير مسعَّر»**: عطلٌ عندنا لا حقيقة عن البضاعة. **والسداد بالصندوق**: `POST /orders/:id/pickup` يفتح **سلّة تلّ مملوءة من الطلب** (`sales.sell` — من يسلّم هو من يسدّد) و`orders.sale_id` الجسر، فلا نسخة ثانية من التسعير والدفع ولا ترقيم ثانٍ. **والحجز يُحرَّر حيث يُخصم الرفّ تماماً** — بمعاملة `paySale` نفسها: تحريرُه عند فتح السلّة يُظهر الصنف متاحاً وهو على الطاولة، وإبقاؤه بعد الدفع يخصمه مرتين فيختفي من البيع وهو موجود، **ولا شيء يفشل بأيٍّ منهما**. والمهلة تقف عند فتح السلّة (`reserved_until = null`)، و**الإلغاء يُرجع الطلب لانتظاره بمهلة جديدة** بدل أن يُجمّد بضاعته للأبد. **والكنس كسِلٌ عند القراءة** لا مهمّةٌ دورية تصير جزءاً ثانياً يجب أن يعمل ليكون الرقم صحيحاً. و`reservation_hours` بصفّ `sales_settings` نفسه، **والحفظ يدمج ولا يكتب كل الحقول** (شاشتان تكتبانه، فجسمٌ ناقص يدهس قيمة الأخرى بصمت). العقد: [docs/rest_api.md](docs/rest_api.md) §27–28 |
