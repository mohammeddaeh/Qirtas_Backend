...
**أول تشغيل**: ٧٠ مساراً — ١٠ منها كانت بلا تصنيف (كلها عامة بالتصميم، وُسمت `publicRoute`)، **و١٧ مفتاحاً مبذوراً لا يفرضه أي مسار** (`orders.*` · `printing.*` · `inventory.*` · `reports.*` — وحدات قيد البناء). ولا بوابة مغلقة للأبد: كل مفتاح مفروض كان مبذوراً.
