...
### المرتجع من الزبون (7-ج، 2026-09-24) · `orders_delivery.md` §٢

**مفتاحٌ مستقل `sales.refund`**: البيع يُدخل مالاً والمرتجع يُخرجه، ومن يقف على الصندوق لا يلزم أن يملك الاثنين. ومنحهما معاً قرارُ إدارةٍ لا افتراضُ كود.

- `GET /sales/settings` · `PUT /sales/settings {return_window_days?, reservation_hours?}` — **الحقلان اختياريان** (شاشتان تكتبان الصفّ نفسه، وإلزامُ الاثنين يجعل كل شاشة تدهس قيمة الأخرى بصمت). **مهلة واحدة للشركة** (قرار 2026-09-24): مهلةٌ لكل فرع تجعل زبوناً اشترى من فرع وأرجع لآخر يواجه قاعدتين ولا يعرف أيّهما تخصّه. و**صفرٌ = لا إرجاع إطلاقاً**، قرارٌ مشروع يُكتب صراحةً لا يُقرأ «بلا حدّ». والصفّ **يُنشأ بأول قراءة** — وإلا رُفض كل إرجاع بيومٍ افتتاحي لأن أحداً لم يزر شاشةً لا يعرف بوجودها.
- `GET /sales/:id/returnable` — **السقف والسعر كلاهما من الخادم**: `unit_refund_syp` هو **المدفوع فعلاً** (مجموع السطر ÷ كميته، أي بعد العرض وبعد حصّة خصم الكاشير)، و`returnable_qty` هو المباع ناقص ما أُرجع سابقاً. طرحُه بالعميل يجعل جهازين يُرجعان القطعة نفسها، وحسابُ السعر هناك يتجاهل حصّة الخصم فيُخرج من الدرج أكثر مما دخله.
- `POST /returns {branch_id, sale_id, lines[{sale_line_id, qty, condition}], refund_method, reason?}`
- `POST /returns/approve {…, email, password}` — **بعد المهلة**: `409 return_window_passed` أولاً، ثم موافقةُ مديرٍ ببريده وكلمة مروره **وحسابه فعّال** على نفس الجهاز، ولا جلسة تُفتح له. والرفض المطلق كان سيجعل المحل يخسر زبوناً دائماً على يوم واحد.
- `GET /returns?branch_id&sale_id` · `GET /returns/:id`

| القرار | ما بُني | لماذا |
|---|---|---|
| حال القطعة | **الكاشير يقوله وهي بيده**: `sellable` تعود للرفّ · `damaged` **لا تدخل الرصيد** والمال يُرَدّ كاملاً | إدخالُ كل مرتجع يجعل الدفتر يقول إن عندك قلماً مكسوراً جاهزاً للبيع، والجرد يكتشفه بعد شهر. وعيبُ البضاعة ليس خطأ الزبون |
| الاسترداد | **الكاشير يختار** نقداً أو رصيداً، و**الرصيد للمسمّى وحده** (`422 return_credit_needs_customer`) | رصيدٌ لمن لا حساب له مالٌ لا يعود إليه أبداً، والعابر يأخذ نقده ويمضي |
| فرع الإرجاع | **أي فرع**، والبضاعة تدخل مخزون **المستلِم** | الرصيد يزيد حيث القطعة فعلاً — وإلا صار بالدفتر قلمٌ بفرعٍ لا وجود له فيه |
| الترقيم | `MZ-R-2026-000001` — **تسلسل مستقل** | تسلسلٌ مشترك يترك بترقيم البيع فجوةً يقرؤها المدقّق فاتورةً مفقودة، وهي مرتجعٌ موجود تماماً |

**والمرتجع ثلاثة أشياء بمعاملة واحدة**: يصرف رقمه · يقيّد الرصيد إن كان الردّ رصيداً · ويُعيد الصالح للرفّ عبر منفذ `core/stock/stock-port.ts`. وأسطر البيع **تُقفَل** بينها وبين الكتابة، فلا يُرجع جهازان القطعة نفسها.

**وحقل `beyond_window` يُحفظ بالصفّ ولا يُشتقّ** من التاريخين: تغيير المهلة لاحقاً كان سيعيد تصنيف مرتجعات قديمة، فيقول تقريرٌ إن استثناءً لم يحدث.

## 28. السلّة والطلب الإلكتروني (8-أ، 2026-09-24) · المرجع: `docs/reference/orders_delivery.md`

**الطلب هنا حجزٌ للاستلام من الفرع، لا توصيل.** الزبون يملأ سلّة، يؤكّدها فتُحجز البضاعة بمهلة، ثم يأتي فيُسدّد **بالصندوق**. والتوصيل موديولٌ لاحق لا تغيير على هذا.

| القرار (2026-09-24) | ما بُني | لماذا |
|---|---|---|
| مكان السلّة | **بالخادم دائماً**، والضيف يسجّل أولاً | سلّةٌ بالجهاز تختفي بتبديل الهاتف وتُعيد الزبون للبداية؛ والحجز وعدٌ باسمٍ، فلا يُعطى لمن لا يُعرف |
| السلّة والمخزون | **السلّة لا تحجز شيئاً** | سلاتٌ متروكة تجمّد المخزون، فيقرأ الكاشير رفّاً ممتلئاً ولا يستطيع بيع قطعة منه |
| التأكيد | **يحجز بمعاملة ذرّية** مع فحصٍ داخل قفل الصفّ | بين ملء السلّة والتأكيد قد يشتري غيره آخر قطعة؛ وبلا القفل يُوعَد اثنان بها |
| انتهاء المهلة | **تُضبط بالإعدادات ويُحرَّر الحجز تلقائياً**، و`0` = بلا مهلة | حجزٌ أبديّ لطلبٍ لن يأتي يُخرج البضاعة من المتجر بلا أن يبيعها أحد. و`0` قرارٌ إداري، لا «تنتهي فوراً» التي تُلغي كل طلب لحظة تأكيده |
| الاستلام | **من الفرع فقط** | التوصيل موديول قائم بذاته (مندوبون، مناطق، أجور) ولا يُبنى ضمناً بطريق الطلب |
| السداد | **بالصندوق عند الاستلام** — الطلب يفتح سلّة تلّ مملوءة منه | نسخةٌ ثانية من التسعير والدفع تنحرف أول تعديل، والترقيم كان سيصير اثنين. الفاتورة واحدة، والجسر `orders.sale_id` |

### السلّة (الزبون بجلسته) — لا صلاحية، والفرع إلزامي بكل نداء

السلّة **واحدة لكل (زبون × فرع)**: الفرع يُنفّذ السطر، وسلّةٌ عابرة للفروع كانت ستعرض أسعار فرعٍ وبضاعة آخر بالشاشة نفسها.

- `GET /cart?branch_id` — **كل شيء يُقرأ الآن**: السعر (قناة `online`، والعروض تُطبَّق) والمتاح (`on_hand − reserved`). سعرٌ محفوظ بالسطر يُري الزبون ثمن الأسبوع الماضي.
- `POST /cart/items?branch_id {variant_id, qty}` — **الإضافة المكرّرة ترفع الكمية**: سطران بخمسة يفوّتان شريحة العشرة، ويقرأ الزبون صنفه مرتين.
- `PATCH /cart/items?branch_id {variant_id, qty}` — الكمية تُوضَع لا تُزاد، و**الصفر يحذف السطر**.
...
### طلباتي (الزبون بجلسته)

- `GET /my-orders?status&page&limit` — **منسوبة للجلسة لا لمعرّفٍ بالاستعلام**.
- `GET /my-orders/:id` — طلبُ غيره **`404` لا `403`**: «ممنوع» تؤكّد أن الرقم صحيح فيُجرَّب الذي يليه.
- `POST /my-orders/:id/cancel {reason?}` — **يحرّر الحجز بنفس المعاملة**.

### الطابور (الموظف)

- `GET /orders?branch_id&customer_id&status` · `GET /orders/:id` — `orders.view`.
- `POST /orders/:id/pickup` — **`sales.sell` لا `orders.view`**: من يسلّم هو من يسدّد، ومفتاح القراءة كان سيفتح سلّاتٍ لا يستطيع صاحبها إقفالها. يُرجع `{order, sale_id}`، و**استدعاؤه مرتين يُعيد السلّة نفسها** لا ثانيةً بنفس البضاعة.
- `POST /orders/:id/cancel {reason?}` — `orders.manage_issues`: إلغاء طلب زبون قرارٌ للفرع لا قراءة.

**والحالات أربع** (`pending_pickup` · `picked_up` · `cancelled` · `expired`) و`next_states` تسافر بالردّ فتُبنى الأزرار منها. **و`expired` ليست `cancelled`**: الأولى تقول إن الزبون لم يأتِ، والثانية إن أحداً قرّر — وجمعُهما يُخفي كم طلباً يضيع بلا أن يلغيه أحد.

### دورة الحجز كاملةً — وأين يُحرَّر بالضبط

| اللحظة | `on_hand` | `reserved` |
|---|---|---|
| ملء السلّة | — | — (السلّة لا تحجز) |
| `POST /cart/checkout` | — | **+الكمية** |
| `POST /orders/:id/pickup` | — | كما هو، **و`reserved_until` يصير `null`** (الزبون واقفٌ عند الصندوق، والساعة تقف) |
| `POST /sales/:id/pay` | **−الكمية** | **−الكمية**، والطلب يصير `picked_up` — **بنفس معاملة الدفع** |
| `POST /sales/:id/void` | — | كما هو، والطلب يعود `pending_pickup` **بمهلة جديدة** |
| الإلغاء أو انتهاء المهلة | — | **−الكمية** |

**وتحريرُ الحجز عند فتح السلّة كان سيُظهر الصنف متاحاً وهو على الطاولة**، فيَعِد به الخادم زبوناً آخر. **وإبقاؤه بعد الدفع كان سيخصم البضاعة مرتين** — مرة من الرفّ ومرة من المتاح — فيكفّ الصنف عن الظهور للبيع وهو موجود. ولا شيء يفشل بأيٍّ منهما: الرصيد صحيح والمتاح كاذب.
