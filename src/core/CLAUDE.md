...
| مفاتيح مخطَّطة لم يعلنها مسار | ℹ️ تُذكَر بالأسماء — **ولا تُبذَر** |

### 🌱 المفتاح يُبذَر يوم يفرضه مسار، لا قبله

قائمة `PERMISSIONS` بـ`seed-core.ts` **خطة**: كُتبت قبل الوحدات ليكون الدور جاهزاً يوم تُبنى. وبذرُ الخطة كاملةً أنتج كتالوجاً يكذب — **١٧ من ٢٧ مفتاحاً لا تحكم شيئاً** وكلها تظهر للمسؤول ليؤشّرها ويمنحها ويصدّقها.

فصار: **الخطة تبقى في الملف، والقاعدة تأخذ الواقع وحده**، والـregistry هو الحَكَم.

| الدالة | ما تفعله الآن |
|---|---|
| `seedPermissions` | تُدخل المفروض فقط، وتحذف من الجدول ما لم يعد أي مسار يفرضه (ومعه منحُه الخاملة — FK بـ`CASCADE`) بسجلّ صريح بالأسماء |
| `seedRoles` | تمنح `role.permissionKeys.filter(isLive)` — دورٌ يخطّط لصلاحية غير مبنيّة يستلمها لاحقاً |
| `seedPermissionDisplayNames` | ترجمات للمفروض فقط |

### 🎚️ الاستثناء الفردي — وراثة / سماح / حظر

الأدوار تجيب «ماذا تفعل هذه الوظيفة». ولا تجيب «…إلا أحمد، لا يحذف». وبلا جواب لذلك، كل استثناء يصير **دوراً بعضو واحد** — حتى يصل النشر إلى أربعين دوراً لا أحد يستطيع وصفها.

`authz_user_permission_overrides` (user, key, effect) — و**الوراثة هي غياب الصف** لا قيمة فيه.

| قرار | لماذا |
|---|---|
| **غير مقيَّد بفرع** | التجاوز يقول شيئاً عن **الشخص** لا عن منصب يشغله في مكان. تقييده كان يطرح سؤالاً بلا جواب حسن: أيهما يغلب، العام أم المقيَّد؟ |
| **يُطبَّق في `findAllEffectivePermissionKeys` وحدها** | كل حارس وكل شاشة و`/users/me` يقرأون تلك الدالة، فلا يختلف جواب الحساب باختلاف السائل |
| **الحظر يُغلق لأعلى** | حظر `roles.view` يسحب `roles.edit` معه — القاعدة الثانية بـ`inference.ts`. البديل يترك حساباً يكتب في شاشة لا يفتحها |
| **مفتاح غير مفروض يُهمَل** | تجاوزٌ لمفتاح حُذف مساره خاملٌ لا قاتل، ويُعرَض بالشاشة كـ«لم يعد مفروضاً» |

**الحماية الوحيدة من النفس**: `PUT /users/:id/overrides` يرفض أن تحظر عن نفسك `users.manage` أو `roles.edit` — الفحص **قبل** الكتابة، إذ لا شيء يُرجَع إليه بعدها.

**قاعدتا الاستنتاج** (`core/authz/inference.ts`، ملف واحد لا غير):
1. كل إجراء يستلزم أقرب `view` معلَن — وإلا «منحته تعديل فرأى قائمة فارغة».
2. حظرُ عرضٍ يحظر كل ما كان يستلزمه — عكس نقيض الأولى.

### 🎯 التفصيل — مفتاح لكل فعل، ومظلّة تجمعها

**كان `users.manage` يحرس ١٣ مساراً**: العرض والإنشاء والتعديل والتعليق والحذف والأرشفة. فمنحُه يعطي كل شيء دفعةً واحدة، **ولا يمكن أن يظهر شيء أو يختفي** — لا يوجد أدقّ منه ليُكشَف.
...
| `users.edit` | `PATCH /users/:id` |
| `users.status` | تعليق · تعطيل · إعادة تفعيل |
| `users.approve` | البتّ في طلبات التسجيل |
| `users.access` | قراءة/كتابة أدواره واستثناءاته |
| `users.delete` | حذف · أرشفة · استرجاع |
| **`users.manage`** | **مظلّة — كل ما سبق، وما يُضاف لاحقاً** |

**ولا هجرة**: كل دور يحمل `users.manage` بقي يعمل كما هو حرفياً، لأن المظلّة تتوسّع إلى كل `users.*` عند الحلّ (`inference.ts` قاعدة صفر). وهي **مفتاح منح لا يُعلنه أي مسار** — `registerPermission` يرفض الفعل `manage` عمداً، إذ إعلانُه على مسار يُجمّد معناه عند الأفعال الموجودة يوم كُتب.
...
```
<module>.view · <module>.create · <module>.edit · <module>.delete   ← الأربعة القياسية
<module>.<فعل خاص>                                                  ← status · approve · access
<module>.manage                                                     ← المظلّة، تلقائية
```

وقاعدة التضمين تعمل معها بلا إعلان: من يملك `edit` يحصل على `view`.

### ✅ صلاحية جديدة = سطر واحد، ولا ملف ثانٍ
...
ordersRouter.post(
  ...
  requirePermission('orders.create', {
    ...
    sensitive: false,                      // اختياري، الافتراضي false
  }),
  ...
);
```

ثم `npm run db:seed`. هذا السطر **هو** الصلاحية: يحرس المسار، ويُنشئ صفّ الكتالوج، ويكتب اسمها بالعربية والإنجليزية في `translation_entries`، **ويمنحها لكل دور خطّط لها** بـ`ROLES`. لا قائمة تُحرَّر، ولا شيء يُتذكَّر.

| من أين تأتي القيمة | الأولوية |
|---|---|
| `display` | من المسار ← ثم من `PERMISSIONS` (خطة الوحدات غير المبنيّة) ← **وإلا البذرة ترفض وتسمّي المفتاح** |
| `module` | مشتقّ من أول مقطع بالمفتاح — لا يُكتب |
| `is_sensitive` | من المسار ← ثم الخطة ← ثم `false`. **يشتدّ ولا يرتخي**: مسار واحد يعلنها حسّاسة يكفي |

فبقيت `PERMISSIONS` بـ`seed-core.ts` **خطةً للوحدات القادمة فقط**، لا مصدراً. ومفتاح مفروض بلا اسم عرض يوقف البذرة برسالة تحمل الشيفرة الجاهزة للصق — بدل شاشة أدوار تعرض `orders.create` كتسمية لا يفهمها أحد.

والحذف آمنٌ لأن الخطة في الملف لا في الجدول: أي مفتاح مقصوص يعود كاملاً بمنحِه في التشغيل التالي.
